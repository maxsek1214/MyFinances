...
@Entity(
    ...
    foreignKeys = [
        ForeignKey(
            ...
            childColumns = arrayOf("accountType_id")
        ),
        ForeignKey(
            entity = IssuerEntity::class,
            parentColumns = arrayOf("id"),
            childColumns = arrayOf("issuer_id")
        ),
        ForeignKey(
            entity = CardTypeEntity::class,
            parentColumns = arrayOf("id"),
            childColumns = arrayOf("cardType_id")
        )
    ],
    indices = [Index(value = arrayOf("issuer_id")), Index(value = arrayOf("cardType_id")), Index(
        value = arrayOf("accountType_id")
    )]
)
